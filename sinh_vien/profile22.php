...
// Truy vấn danh sách lịch thực tập theo ca của sinh viên
$lich_thuc_tap_list = [];
if ($sinh_vien['stt_sv']) {
    $sql = "SELECT lt.ngay_thuc_tap, lt.ca_lam, lt.thoi_gian_ca, lt.danh_gia, td.tieu_de
            FROM lich_thuc_tap lt
            JOIN ung_tuyen ut ON lt.ma_dang_ky = ut.id
            JOIN tuyen_dung td ON ut.ma_tuyen_dung = td.ma_tuyen_dung
            WHERE lt.stt_sv = ?
            ORDER BY lt.ngay_thuc_tap ASC";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        die("Lỗi chuẩn bị truy vấn lịch thực tập: " . $conn->error);
    }
    $stmt->bind_param("i", $sinh_vien['stt_sv']);
    $stmt->execute();
    $result = $stmt->get_result();
    $lich_thuc_tap_list = $result->fetch_all(MYSQLI_ASSOC);
    $stmt->close();
}
...
// Truy vấn danh sách điểm đánh giá từ pdf_nhan
$danh_gia_list = [];
$errors = [];
if ($sinh_vien['stt_sv']) {
        ...
        if (empty($errors)) {
            ...
            if (!$stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    // Tạo CSRF token mới sau khi gửi thành công
            ...
            <div class="section-content">
                ...
                <div class="lich-thuc-tap-list">
                    ...
                    <table class="status-table">
                        ...
                        <tbody>
                            <?php if (empty($lich_thuc_tap_list)): ?>
                                <tr>
                                    <td colspan="5">Bạn chưa có lịch thực tập nào.</td>
                                </tr>
                            <?php else: ?>
