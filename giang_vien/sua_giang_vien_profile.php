...
// Lấy thông tin giảng viên dựa trên so_hieu_giang_vien hoặc ho_ten từ session
...
// Lưu so_hieu_giang_vien từ kết quả truy vấn
$so_hieu_giang_vien = $giang_vien['so_hieu_giang_vien'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($ho_ten) || empty($email) || empty($so_dien_thoai)) {
        ...
        $sql_check = "SELECT 1 FROM giang_vien WHERE email = ? AND so_hieu_giang_vien != ?";
        $stmt_check = $conn->prepare($sql_check);
        $stmt_check->bind_param("ss", $email, $so_hieu_giang_vien);
        $stmt_check->execute();
        $stmt_check->store_result();
        if ($stmt_check->num_rows > 0) {
            $update_error = "Email đã được sử dụng bởi giảng viên khác!";
        } else {
            // Cập nhật dựa trên so_hieu_giang_vien
            $sql = "UPDATE giang_vien SET ho_ten = ?, email = ?, so_dien_thoai = ?, khoa = ? WHERE so_hieu_giang_vien = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                $update_error = "Lỗi chuẩn bị truy vấn: " . $conn->error;
            } else {
                $stmt->bind_param("sssss", $ho_ten, $email, $so_dien_thoai, $khoa, $so_hieu_giang_vien);
                if ($stmt->execute()) {
                    if ($stmt->affected_rows > 0) {
                        $update_success = "Cập nhật thông tin thành công!";
                        $_SESSION['name'] = $ho_ten; // Cập nhật lại session
                        $_SESSION['so_hieu_giang_vien'] = $so_hieu_giang_vien;
                        header("Location: ./giang_vien_profile.php");
                        exit();
                    } else {
                        $update_error = "Không có thay đổi nào được thực hiện hoặc giảng viên không tồn tại!";
                    }
                } else {
                    $update_error = "Lỗi khi cập nhật: " . $stmt->error;
                    error_log("Lỗi cập nhật giảng viên: " . $stmt->error, 3, '../errors.log');
                }
                $stmt->close();
            }
        }
...
<head>
    ...
    <title>SMAS - Cập nhật thông tin giảng viên</title>
...
<body>
    <div class="header">
        ...
        <div class="nav">
            <a href="./giaodien_giangvien.php">Trang chủ</a>
    ...
    <div class="main-content">
        <div class="section">
            <div class="section-header">
                <div>Cập nhật thông tin giảng viên</div>
                <div>▲</div>
            </div>
            <div class="section-content">
                <?php if (!empty($update_success)): ?>
                    <div class="message success"><?php echo htmlspecialchars($update_success); ?></div>
                ...
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="<?php echo htmlspecialchars($giang_vien['so_hieu_giang_vien']); ?>" disabled>
                    </div>
                    <div class="form-group">
                        <label>Họ và tên <span class="required">*</span></label>
                        <input type="text" name="ho_ten" class="form-control" value="<?php echo htmlspecialchars($giang_vien['ho_ten']); ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Email <span class="required">*</span></label>
                        <input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($giang_vien['email']); ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Số điện thoại <span class="required">*</span></label>
                        <input type="tel" name="so_dien_thoai" class="form-control" value="<?php echo htmlspecialchars($giang_vien['so_dien_thoai']); ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Khoa</label>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="./giang_vien_profile.php" class="cancel-btn"><i class="fas fa-times"></i> Hủy</a>
                    </div>
                </form>
            </div>
        </div>
